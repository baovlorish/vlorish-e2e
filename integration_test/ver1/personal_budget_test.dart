    ...
    testWidgets('Personal Budget test', (tester, [String? context]) async {
      ...
      try {
        await htLogdDirect(
            ...
            '',
            'STARTED');
        await personalBudgetScreen.verifyShowPersonalDefautPageAfterSignIn(tester);
      ...
      try {
        await htLogdDirect('BAR_T134 Check that the user is able to Collapse and Expand categories',
            '', 'STARTED');
        await dashboardScreen.clickProfileIcon(tester);
        await personalBudgetScreen.clickPersonalTab(tester);
      ...
      try {
        ...
        await htLogd(tester, 'Failed BAR_T140 Check List of Categories', '', 'FINISHED');
      }
      ...
      try {
        ...
        await personalBudgetScreen.clickMonthly(tester);
        await personalBudgetScreen.verifyBudgetMonthlyPage(tester);
        await personalBudgetScreen.verifyYearOnBudgetMonthly(currentYear, tester);
        ...
        await personalBudgetScreen.verifyYearOnBudgetMonthly(currentYear, tester);
        await personalBudgetScreen.clickLeftArrowBtn(tester);
        await personalBudgetScreen.verifyYearOnBudgetMonthly(previousYear, tester);
        await htLogd(
            tester,
            'BAR_T157 Check that user is able to switch months/years on Budget Personal Monthly page',
            '',
      ...
      try {
        ...
        await personalBudgetScreen.clickLeftArrowBtn(tester);
        await personalBudgetScreen.verifyYearOnBudgetAnnual(currentYear, tester);
        await personalBudgetScreen.clickLeftArrowBtn(tester);
        await personalBudgetScreen.verifyYearOnBudgetAnnual(previousYear, tester);
        await htLogd(
            tester,
            'BAR_T222 User can change Current selected year on previous year or any further year for budget planning',
            '',
