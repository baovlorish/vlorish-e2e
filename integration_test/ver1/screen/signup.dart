...
class SignUpScreenTest {
  ...
  Future<void> clickBtnSignIn(WidgetTester tester, {String context = ""}) async {
    await tester.pumpAndSettle();
    final btnSignIn = find.text('Sign-in').first;
    await tapSomething(tester, btnSignIn, addContext(context, "click Btn SignIn"));
  ...
  Future<void> inputPassword(String passUser, WidgetTester tester, {String context = ''}) async {
    ...
    await writeSomething(tester, pass, passUser, addContext(context, 'Input password'));
  ...
  Future<void> inputConfirmPassword(String passUser, WidgetTester tester,
    ...
    await writeSomething(tester, pass, passUser, addContext(context, 'Input Confirm password'));
    await tester.enterText(pass, passUser);
  }
  ...
  Future<void> verifyNotCurrentPassword(String passUser, WidgetTester tester,
    ...
    await htExpect(tester, currentPass.toString(), passUser,
        reason: ("Verify-" + context + "-" + 'Current password is not this value ' + passUser));
  }

  Future<void> clickButtonNext(WidgetTester tester, {String context = ''}) async {
    await tester.pumpAndSettle();
  ...
  Future<void> clickEyeConfirmPassword(WidgetTester tester, {String context = ""}) async {
    await tester.pumpAndSettle(const Duration(seconds: 2));
    final btnNext = find.byType(IconButton).last;
    await tapSomething(tester, btnNext, addContext(context, "Click on Eye in confirm password"));
    await tester.pumpAndSettle();
  }
  ...
  Future<void> verifyPasswordPage(WidgetTester tester, {String context = ''}) async {
    await tester.pumpAndSettle(const Duration(seconds: 2));
  ...
  Future<void> clickAgreeAndContinueBtn(WidgetTester tester, {String context = ''}) async {
    await tester.pumpAndSettle();
    final btnAgree = find.text('Agree & Continue').first;
    await tester.ensureVisible(btnAgree);
    await tapSomething(tester, btnAgree, addContext(context, 'click Btn Agree & Continue'));
    await tester.pumpAndSettle(const Duration(seconds: 2));
  }
  ...
  Future<void> clickAndVerifyPrivacyLink(String homeURL, WidgetTester tester,
    ...
    await tapSomething(tester, privacyLink, addContext(context, 'click Privacy link'));
    await tester.pumpAndSettle(const Duration(seconds: 2));
    var currentUrl = Uri.base.toString();
    print(currentUrl);
  ...
  Future<void> verifySignUpPage(WidgetTester tester, {String context = ''}) async {
    await tester.pumpAndSettle(const Duration(seconds: 6));
    expect(find.text('Please enter your email address to create an account'), findsOneWidget);
    await htExpect(
  ...
  Future<void> verifySigupMailCodePage(WidgetTester tester, {String context = ''}) async {
    await tester.pumpAndSettle(const Duration(seconds: 2));
    ...
    await htExpect(tester, find.text('We sent an email with a code to '), findsOneWidget,
        reason: ("Verify-" + context + '- Text We sent an email with a code is visible'));
    await tester.pumpAndSettle(const Duration(seconds: 2));
  }
  ...
  Future<void> inputConfirmCodeEmail(WidgetTester tester, {String context = ''}) async {
    ...
    await writeSomething(
        tester, code, '111111', addContext(context, 'Input code confirm your email'));
    // await tester.enterText(code, '111111');
    await tester.pumpAndSettle();
  }

  Future<void> verifySigupPersonalInfoPage(WidgetTester tester, {String context = ''}) async {
    await tester.pumpAndSettle(const Duration(seconds: 10));
    await htExpect(tester, find.text('We want to know you better'), findsOneWidget,
        reason: ("Verify-" + context + '- Personal Info Page Title is visible'));
    await htExpect(tester, find.text('Please add some details about yourself'), findsOneWidget,
        reason: ("Verify-" + context + '- Text Please add some details about yourself is visible'));
    await tester.pumpAndSettle(const Duration(seconds: 2));
  }
  ...
  Future<void> inputFirstName(String fName, WidgetTester tester, {String context = ''}) async {
    await tester.pumpAndSettle();
    final firstName = find.byType(InputItem).first;
    await writeSomething(tester, firstName, fName, addContext(context, 'Input first name'));
    await tester.enterText(firstName, fName);
  }

  Future<void> inputLastName(String lName, WidgetTester tester, {String context = ''}) async {
    await tester.pumpAndSettle();
    final lastName = find.byType(InputItem).at(1);
    await writeSomething(tester, lastName, lName, addContext(context, 'Input last name'));
  ...
  Future<void> clickDropdownButton(WidgetTester tester, {String context = ''}) async {
    await tester.pumpAndSettle(const Duration(seconds: 5));
    ...
    await tapSomething(
        tester, imageIconFinder.first, addContext(context, 'Click on dropdown button'));
    await tester.pumpAndSettle();
  }
  ...
  Future<void> verifySelectDropdown(String valueSelect, WidgetTester tester,
    ...
    await tapSomething(tester, valueFinder, addContext(context, 'Click on value $valueSelect'));
    await tester.pumpAndSettle();
  }

  Future<void> clickStepEmailConfirmation(WidgetTester tester, {String context = ''}) async {
    await tester.pumpAndSettle(const Duration(seconds: 2));
    final btnSignIn = find.text('Email confirmation').first;
    await tapSomething(tester, btnSignIn, addContext(context, "click Step Email Confirmation"));
    await tester.pumpAndSettle();
  }

  Future<void> clickStepPersonalInfo(WidgetTester tester, {String context = ''}) async {
    await tester.pumpAndSettle(const Duration(seconds: 2));
    final btnSignIn = find.text('Personal Info').first;
    await tapSomething(tester, btnSignIn, addContext(context, "click step Personal Info"));
    await tester.pumpAndSettle();
  }
}
