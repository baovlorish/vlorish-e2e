...
void main() async {
  ...
  group('Profile Page', () {
    testWidgets('Profile test', (tester, [String? context]) async {
      ...
      try {
        await htLogdDirect(
            'BAR_T103 New Password field contains at least 8 characters', '', 'STARTED');
        await dashboardScreen.clickProfileIcon(tester);
        await profileScreen.verifyProfilePage(tester);
        await profileScreen.inputUpdatePassword(passLogin, 'Test13@', tester);
        await profileScreen.clickUpdatePasswordButton(tester);
        await profileScreen.verifyMessageErrorIsVisible('contains at least 8 characters', tester,
            context: context);
        await htLogd(
            tester, 'BAR_T103 New Password field contains at least 8 characters', '', 'FINISHED');
      } catch (e) {
        await htLogd(tester, 'Failed BAR-T103 New Password field contains at least 8 characters',
            '', 'FINISHED');
      }

      try {
        await htLogdDirect('BAR_T104 User can make password visible after clicking on "eye" button',
            '', 'STARTED');
        await dashboardScreen.clickProfileIcon(tester);
        await profileScreen.verifyProfilePage(tester);
      ...
      try {
        await htLogdDirect(
            'BAR_T106 User can enter max 128 characters in Password fields', '', 'STARTED');
        final pass128 =
        ...
        await dashboardScreen.clickProfileIcon(tester);
        await profileScreen.verifyProfilePage(tester);
        await profileScreen.inputUpdatePassword(passLogin, passGreaterThan128, tester);
        await profileScreen.clickEyePassword(1, tester);
        await profileScreen.verifyNewPasswordMax128Char(pass128, passGreaterThan128, tester);
        await htLogd(tester, 'BAR_T106 User can enter max 128 characters in Password fields', '',
            'FINISHED');
      } catch (e) {
        await htLogd(tester, 'Failed BAR-T106 User can enter max 128 characters in Password fields',
            '', 'FINISHED');
      }
      ...
      try {
        await htLogdDirect(
            ...
            'STARTED');
        await dashboardScreen.clickProfileIcon(tester);
        await profileScreen.inputUpdatePassword(passLogin, 'testPassword@', tester);
        await profileScreen.clickUpdatePasswordButton(tester);
        await profileScreen.verifyMessageErrorIsVisible(
            'contains at least one number (0-9) and a symbol', tester,
            context: context);
      ...
      try {
        await htLogdDirect(
            ...
            'STARTED');
        await dashboardScreen.clickProfileIcon(tester);
        await profileScreen.inputUpdatePassword(passLogin, 'password@123', tester);
        await profileScreen.clickUpdatePasswordButton(tester);
        await profileScreen.verifyMessageErrorIsVisible(
      ...
      try {
        await htLogdDirect(
            'BAR_T111 User can not update password if current password is invalid', '', 'STARTED');
        await dashboardScreen.clickProfileIcon(tester);
        await profileScreen.inputUpdatePassword('Test@123456', 'Pass@123456', tester);
        await profileScreen.clickUpdatePasswordButton(tester);
        ...
        await htLogd(tester, 'BAR_T111 User can not update password if current password is invalid',
            '', 'FINISHED');
      } catch (e) {
        await htLogd(
            tester,
      ...
      try {
        await htLogdDirect(
            'BAR_T117 User sees error message if password fields are empty', '', 'STARTED');
        await dashboardScreen.clickProfileIcon(tester);
        ...
        await profileScreen.verifyMessageErrorIsVisible(
            'contains at least one number (0-9) and a symbol', tester);
        await htLogd(tester, 'BAR_T117 User sees error message if password fields are empty', '',
            'FINISHED');
      } catch (e) {
        await htLogd(tester, 'Failed BAR-T117 User sees error message if password fields are empty',
            '', 'FINISHED');
      }

      try {
        await htLogdDirect('BAR_T118 User sees error message if one of the password field is empty',
            '', 'STARTED');
        await dashboardScreen.clickProfileIcon(tester);
      ...
      try {
        await htLogdDirect('BAR_T99 Check that user can update password', '', 'STARTED');
        await personalBudgetScreen.clickPersonalTab(tester);
        await dashboardScreen.clickProfileIcon(tester);
        await profileScreen.verifyProfilePage(tester);
        await profileScreen.inputUpdatePassword(passLogin, 'Hello@1234', tester);
        await profileScreen.clickUpdatePasswordButton(tester);
        ...
        await signInScreen.verifyErrorMessage(
            'The password is incorrect. Please check the password and try again', tester,
            context: context);
        ...
        await htLogd(tester, 'BAR_T99 Check that user can update password', '', 'FINISHED');
      } catch (e) {
        await htLogd(tester, 'Failed BAR-T99 Check that user can update password', '', 'FINISHED');
      }
    });
  });
}
