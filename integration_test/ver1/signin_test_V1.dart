  ...
  group('Authentication Test', () {
    testWidgets('SignIn test', (tester, [String? context]) async {
      ...
      try {
        ...
        await signInScreen.inputEmailAndPassword(emailLogin, 'Test1@123456', tester);
      ...
      try {
        await htLogdDirect('BAR_T57 Login Web with empty email and empty password', '', 'STARTED');
        await dashboardScreen.clickLogoText();
        await signInScreen.inputEmailAndPassword('', '', tester);
        await signInScreen.clickLoginButton(tester);
        await signInScreen.verifyErrorMessage('Please, enter your email', tester);
        await signInScreen.verifyErrorMessage('Please enter your password', tester);
        await htLogd(
            tester, 'BAR_T57 Login Web with empty email and empty password', '', 'FINISHED');
      } catch (e) {
        await htLogd(
            tester, 'Failed BAR-T57 Login Web with empty email and empty password', '', 'FINISHED');
      }

      try {
        await htLogdDirect('BAR_T58 User cannot login with empty one of the fields', '', 'STARTED');
        await dashboardScreen.clickLogoText();
        await signInScreen.inputEmailAndPassword('', 'Abcd123!@#', tester);
        await signInScreen.clickLoginButton(tester);
        await signInScreen.verifyErrorMessage('Please, enter your email', tester);
        await signInScreen.inputEmailAndPassword(getRandomString(12) + '@gmail.com', '', tester);
        await signInScreen.clickLoginButton(tester);
        await signInScreen.verifyErrorMessage('Please enter your password', tester);
        await htLogd(
            tester, 'BAR_T58 User cannot login with empty one of the fields', '', 'FINISHED');
      } catch (e) {
        await htLogd(tester, 'Failed BAR-T58 User cannot login with empty one of the fields', '',
            'FINISHED');
      }
      ...
      try {
        ...
        await signInScreen.verifyErrorMessage(
            ...
            tester,
            context: context); // status is pending, will check again after enable to connect DB
        await htLogd(
            tester,
            'BAR_T60 User sees error message after entering incorrect password 2 times',
            '',
      ...
      try {
        ...
        await signInScreen.verifyErrorMessage(
            ...
            tester);
        for (int i = 0; i < 5; i++) {
          await signInScreen.inputPassword(getRandomString(3) + getRandomNumber(3) + 'aB@', tester);
          await signInScreen.clickLoginButton(tester, context: context);
        }
      ...
      try {
        await htLogdDirect(
            'BAR_T64 User can make password visible after tap on “eye“ button', '', 'STARTED');
        await dashboardScreen.clickLogoText();
        await signInScreen.inputEmailAndPassword('', 'Test123', tester, context: context);
        await signInScreen.clickEyePassword(tester, context: context);
        await signInScreen.verifyPasswordShow('Test123', tester);
        await htLogd(tester, 'BAR_T64 User can make password visible after tap on “eye“ button', '',
            'FINISHED');
      } catch (e) {
        await htLogd(
            tester,
      ...
      try {
        ...
        await htLogdDirect('BAR-T67 User can edit visible password', '', 'STARTED');
        await dashboardScreen.clickLogoText();
        await signInScreen.inputEmailAndPassword(email67, 'test123!ABC', tester, context: context);
        await signInScreen.clickEyePassword(tester, context: context);
        await signInScreen.verifyPasswordShow('test123!ABC', tester);
        await signInScreen.inputEmailAndPassword(email67, '123', tester, context: context);
        await signInScreen.clickLoginButton(tester, context: context);
        await signInScreen.verifyErrorMessage(
            ...
            tester,
            context: context);
        await htLogd(tester, 'BAR-T67 User can edit visible password', '', 'FINISHED');
      } catch (e) {
        await htLogd(tester, 'Failed BAR-T67 User can edit visible password', '', 'FINISHED');
      }

      try {
        await htLogdDirect('BAR_T68 User can login with visible password', '', 'STARTED');
        await dashboardScreen.clickLogoText();
        ...
        await htLogd(tester, 'BAR_T68 User can login with visible password', '', 'FINISHED');
      } catch (e) {
        await htLogd(tester, 'Failed BAR-T68 User can login with visible password', '', 'FINISHED');
      }
    });
  });
}
